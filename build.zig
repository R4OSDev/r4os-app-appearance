...
pub fn build(b: *std.Build) void {
    ...
    _ = sdk.addR4MFWithOptions(b.path("module.R4MF"), .{
        .zig_module_roots = &.{libraries_dep.namedLazyPath("r4std_zig_binding"), libraries_dep.namedLazyPath("r4img_zig_binding"), libraries_dep.namedLazyPath("r4gfx_desktop_outputs"), libraries_dep.namedLazyPath("r4gfx_zig_binding")},
    });
    ...
    const colors = b.createModule(.{ .root_source_file = libraries_dep.namedLazyPath("r4gfx_zig_binding"), .target = b.graph.host, .optimize = .Debug });
    colors.addImport("r4os", host); model_tests.root_module.addImport("r4gfx", colors);
    const settings = b.createModule(.{ .root_source_file = libraries_dep.namedLazyPath("r4std_zig_binding"), .target = b.graph.host, .optimize = .Debug });
    settings.addImport("r4os", host); model_tests.root_module.addImport("r4std", settings);
